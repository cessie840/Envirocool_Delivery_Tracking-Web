...
import AdminLayout from "./AdminLayout"; // Using AdminLayout for UserManagement
import { FaArrowLeft } from "react-icons/fa";
import { ToastHelper } from "./helpers/ToastHelper";

const UserCreatePersonnelAccount = () => {
  ...
  useEffect(() => {
    document.title = "Create Personnel Account";
    if (errorMessage) {
  ...
  const handleSubmit = async (e) => {
    ...
    if (!/^09\d{9}$/.test(formData.contactNumber)) {
      setErrorMessage("Contact number must start with '09' and be exactly 11 digits.");
      return;
    }

    if (!/\S+@\S+\.\S+/.test(formData.email)) {
      setErrorMessage("Please enter a valid email address (e.g., example@email.com).");
      return;
    }
  ...
  return (
    <AdminLayout title="Create Delivery Personnel Account">
      ...
      <div className="add-delivery-form-container mt-5 mx-auto">
        <div className="form-card mx-auto">
          ...
          <form className="personnel-form" onSubmit={handleSubmit}>
            ...
            {errorMessage && (
              <div className="text-danger fw-bold text-center my-3">{errorMessage}</div>
            )}
          </form>
        </div>
...
export default UserCreatePersonnelAccount;
